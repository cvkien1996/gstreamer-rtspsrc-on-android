...
typedef struct _CustomData {
    ...
    GMainLoop *main_loop;   /* GLib main loop, dua ra lam bien toan cuc */
...
static void *app_function (void *userdata) {
  ...
  /* Build pipeline */
  //data->rtspaddress = "192.168.1.7:8554/test";
  gchar *descr = g_strconcat("rtspsrc location=rtsp://",data->rtspaddress," latency=10 ! rtph264depay ! avdec_h264 ! videoconvert ! autovideosink",NULL);
  //data->pipeline = gst_parse_launch("rtspsrc location=rtsp://192.168.1.7:8554/test latency=10 ! rtpjpegdepay ! jpegdec ! videoconvert ! autovideosink", &error);
...
static void gst_native_init (JNIEnv* env, jobject thiz, jstring address) {
  ...
  jboolean iscopy;
  data->rtspaddress = (*env)->GetStringUTFChars(env,address,&iscopy);
